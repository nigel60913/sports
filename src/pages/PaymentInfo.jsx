...
export default function PaymentInfo() {
  ...
  return (
    <div className="space-y-5">
      <h1 className="font-display text-3xl font-bold text-ink">匯款資訊</h1>
      <p className="text-base text-ink/50">設定每個人的收款方式，分帳時會直接顯示給付款人看</p>
...
function MemberPaymentCard({ member, open, onToggle, onSave }) {
  ...
  const uploadQr = async (methodId, file) => {
    ...
    setUploading(true)
    try {
      const dataUrl = await fileToCompressedDataURL(file, 480, 0.8)
  ...
  return (
    <div className="bg-white rounded-2xl shadow-card overflow-hidden">
      <button onClick={onToggle} className="w-full flex items-center justify-between px-4 py-3.5">
        <span className="font-medium text-base text-ink">{member.name}</span>
        <div className="flex items-center gap-2">
          <span className="text-sm text-ink/40">{methods.length} 種收款方式</span>
        ...
        <div className="px-4 pb-4 space-y-3 border-t border-black/5 pt-3">
          {methods.map(mth => (
            <div key={mth.id} className="flex items-center gap-3 bg-paper rounded-xl p-3">
              <QrImage src={mth.qrcodeUrl} label={`${member.name} · ${mth.type}`} size="lg" />
              <div className="flex-1 min-w-0">
                <div className="text-sm font-semibold text-ink">{mth.type}</div>
                <div className="text-sm text-ink/50 truncate">{mth.account}</div>
                <label className="text-sm text-accent cursor-pointer font-medium">
              ...
              <button onClick={() => removeMethod(mth.id)} className="text-ink/30 hover:text-red-500"><Trash2 size={18} /></button>
            </div>
          ))}
          ...
          <div className="flex gap-2">
            ...
            <input value={form.account} onChange={e => setForm(f => ({ ...f, account: e.target.value }))}
              placeholder="帳號 / Line ID" className="flex-1 rounded-lg border border-black/10 px-3 py-2 text-sm" />
            <button onClick={addMethod} className="bg-orange text-white rounded-lg px-3"><Plus size={18} /></button>
          </div>
          {uploading && <div className="text-sm text-ink/40">上傳中…</div>}
        </div>
      )}
    </div>
  )
