...
import { Home, Wallet, ClipboardList, Users, QrCode, Trophy } from 'lucide-react'
...
export default function Layout() {
  return (
    <div className="min-h-screen flex flex-col md:flex-row">
      <aside className="hidden md:flex md:flex-col md:w-56 border-r border-black/5 bg-white p-4 gap-1">
      ...
      <header className="md:hidden flex items-center gap-2 px-4 py-3 bg-white border-b border-black/5 sticky top-0 z-20">
        <BrandMark size={30} />
        <span className="font-display text-lg font-bold text-ink">運動啦</span>
      </header>

      <main className="flex-1 pb-20 md:pb-8 px-4 py-5 md:px-8 md:py-8 max-w-3xl w-full mx-auto">
        <Outlet />
      </main>
      ...
      <nav className="md:hidden fixed bottom-0 inset-x-0 bg-white border-t border-black/5 flex justify-around py-1.5 z-20">
        {NAV.map(({ to, label, icon: Icon, end }) => (
          <NavLink key={to} to={to} end={end}
            className={({ isActive }) =>
              `flex flex-col items-center gap-0.5 px-2 py-1.5 rounded-lg text-[11px] font-medium ${
